...
func TestNewNode(t *testing.T) {
	id := node.NewNodeID([]byte("test_node_data"))
	address := net.ParseIP("192.168.1.1")
	port := uint16(8080)

	testNode := node.NewNode(id, address, port)
...
func TestDistance(t *testing.T) {
	id1 := node.NewNodeID([]byte("node_1"))
	id2 := node.NewNodeID([]byte("node_2"))
	address1 := net.ParseIP("192.168.1.1")
	address2 := net.ParseIP("192.168.1.2")
	port := uint16(8080)

	node1 := node.NewNode(id1, address1, port)
	node2 := node.NewNode(id2, address2, port)
...
func TestSameNodeDistance(t *testing.T) {
	id := node.NewNodeID([]byte("same_node_data"))
	address := net.ParseIP("192.168.1.1")
	port := uint16(8080)
